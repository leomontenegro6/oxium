...
if(isset($_GET['montar_tabela']) && $_GET['montar_tabela'] === true){
	...
	<div <?php echo $atributo_id ?> class="<?php echo $classe_tabela ?>" data-pagina="<?php echo $pagina ?>"
		...
		<button type="button" onclick="abrirPagina('jogo_form.php')" class="btn btn-primary pull-right">
		...
		<table>
			...
			<tfoot>
				<tr>
					<th colspan="5">Total de jogos: <span class="total_consulta"></span></th>
				</tr>
			</tfoot>
		</table>
		<div class="acoes">
			<button type="button" title="Editar" data-pagina="jogo_form.php" data-parametros="id={id}">
				<i class="fa fa-edit fa-lg"></i>
			</button>
			<button type="button" title="Excluir" data-onclick="apagaRegistro('jogo_crud.php', {id}, 'Excluir este jogo?', true)" class="btn btn-danger">
	...
	$total_geral = $jogo->getTotal();
	$total_consulta = $jogo->getTotalByListagem($nome_lista, $plataforma_lista);
	...
	$jogo_rs = $jogo->getByListagem($nome_lista, $plataforma_lista, $ordenacao, $filtragem, $limit, $offset);
	...
	$dados_tabela = tabela::formatarDadosTabela($jogo_rs, $colunas, $dados_coluna);
